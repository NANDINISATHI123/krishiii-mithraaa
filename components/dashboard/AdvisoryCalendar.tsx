...
const TaskItem = React.memo(({ task, isDone, isPending, onToggle, language, t }: {
    task: CalendarTask;
    isDone: boolean;
    isPending: boolean;
    ...
    return (
        <div className="flex items-start gap-4 p-3 rounded-lg border dark:border-gray-700">
            ...
            <button
                ...
                title={isPending ? t('pending_sync_status') : (isDone ? 'Mark as not done' : 'Mark as done')}
            >
                {isPending ? (
...
const AdvisoryCalendar = () => {
    ...
    }, [user, isOnline, statuses, refreshPendingCount]);
                    ...
                    <div className="space-y-4">
                        {tasksForCurrentMonth.length > 0 ? tasksForCurrentMonth.map(task => (
                            <TaskItem
                                key={task.id}
                                task={task}
                                isDone={statuses[task.id] || false}
                                isPending={pendingUpdates.has(task.id)}
